...
class CTkTable(customtkinter.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.binded_objects = []
        self.widgets = {}  # Add this to track custom widgets
    ...
    def draw_table(self, **kwargs):
        """Draw the table, handling both text and widget values"""
        for i in range(self.rows):
            for j in range(self.columns):
                ...
                # Get the value from the values list
                try:
                    if self.orient == "horizontal":
                        cell_data = self.values[i][j]
                    else:
                        cell_data = self.values[j][i]
                except IndexError:
                    cell_data = {"value": " "}
                
                # Handle dictionary format for cell data
                if isinstance(cell_data, dict):
                    value = cell_data.get("value", " ")
                    fg_color = cell_data.get("fg_color", None)
                ...
                if isinstance(value, customtkinter.CTkBaseClass):
                    # Store widget reference
                    self.frame[i,j] = value
                    ...
                    # Place the widget
                    value.grid(row=i, column=j, padx=padx, pady=pady, sticky="nsew")
                    if fg_color:
                        value.configure(fg_color=fg_color)
                    if bg_color:
                        value.configure(bg_color=bg_color)
                else:
                    # Handle text values (existing code)
                    if value=="":
                        value = " "
    ...
    def delete_row(self, index=None):
        ...
        self.draw_table()
        self.update_data()
        # Clean up widgets in the row
        for col in range(self.columns):
            if (index, col) in self.widgets:
                self.widgets[(index, col)].destroy()
    ...
    def delete_column(self, index=None):
        ...
        self.draw_table()
        self.update_data()
        # Clean up widgets in the column
        for row in range(self.rows):
            if (row, index) in self.widgets:
                self.widgets[(row, index)].destroy()
                del self.widgets[(row, index)]
    ...
    def delete(self, row, column, **kwargs):
        ...
        if (row, column) in self.widgets:
            self.widgets[(row, column)].destroy()
            del self.widgets[(row, column)]
        if self.write:
            self.frame[row,column].delete(0, customtkinter.END)
            self.frame[row,column].configure(**kwargs)
        else:     
    ...
    def insert_widget(self, row, column, widget, **kwargs):
        """Insert a custom widget into a specific cell"""
        if (row, column) in self.frame:
            self.frame[row, column].destroy()
        
        # Store widget reference
        self.frame[row, column] = widget
        ...
        self.inside_frame.grid_rowconfigure(row, weight=1)
        self.inside_frame.grid_columnconfigure(column, weight=1)
        
        # Update values
        if row >= len(self.values):
            self.values.append([None] * self.columns)
        if column >= len(self.values[row]):
            self.values[row].append(None)
        self.values[row][column] = widget